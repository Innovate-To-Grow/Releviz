...
describe("HomePage", () => {
  ...
  test("keeps the public explanation visible while auth is loading", () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: "Find a time that works for everyone.",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Create a scheduling poll" }),
    ).toBeDisabled();
  });
  ...
  test("explains the product and shows signed-out account actions", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "How Releviz works" }),
    ).toBeInTheDocument();
  });
  ...
  test("starts account creation before an anonymous organizer creates a poll", () => {
    ...
    expect(push).toHaveBeenCalledWith("/login?next=%2Fcreate");
  });
