...
describe("ScheduleChannelEditor", () => {
  test("switches channels and copies into an empty target immediately", async () => {
    ...
    expect(screen.getByTestId("channel-grid-In-Person")).toHaveTextContent("1,0.5");
  ...
  test("confirms before replacing a non-empty target", async () => {
    ...
    expect(screen.getByTestId("channel-grid-Virtual")).toHaveTextContent("0.5,0");
    await userEvent.click(screen.getByRole("button", { name: "Copy Virtual to In-Person" }));
    expect(screen.getByRole("alertdialog")).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "Cancel" }));
    expect(onCopy).not.toHaveBeenCalled();

    await userEvent.click(screen.getByRole("button", { name: "Copy Virtual to In-Person" }));
    await userEvent.click(screen.getByRole("button", { name: "Replace schedule" }));
