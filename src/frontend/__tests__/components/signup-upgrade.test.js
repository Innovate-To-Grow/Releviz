...
jest.mock("@/lib/api/auth", () => ({
  startTemporaryUpgradeRegistration: (...args) => startTemporaryUpgradeRegistration(...args),
}));
...
describe("temporary-account upgrade registration", () => {
  ...
  test("loads and locks the server-bound temporary identity without putting email in the URL", async () => {
    ...
    expect(
      screen.getByText("This email is fixed so your existing event responses stay connected.")
    ).toBeInTheDocument();
    ...
    expect(startTemporaryUpgradeRegistration.mock.calls[0][1]).not.toHaveProperty("email");
  ...
  test("blocks registration when the restricted temporary session cannot be loaded", async () => {
    fetchTempAccessSession.mockRejectedValueOnce(new Error("No temporary session"));
  ...
  test("blocks an incomplete upgrade URL before making a session request", async () => {
    searchParams = new URLSearchParams("upgrade=temporary&next=%2Fevent%3Fcode%3DABC123");

    render(<SignupPage />);

    expect(await screen.findByRole("alert")).toHaveTextContent("upgrade link is incomplete");
