...
from apps.scheduling.services.events import EventManagementError, parse_event_configuration
from apps.scheduling.services.finalization import FinalizationError
from apps.scheduling.services.finalization.delivery import ensure_final_delivery_request
from apps.scheduling.services.slots import SlotConfigurationError, validate_minute_configuration
from apps.scheduling.views.helpers import organizer_response_write_error
from apps.scheduling.views.operations import _calendar_job_sequence, _retryable_job_ids
...
class ScaleOperationEdgeTests(TestCase):
    ...
    def test_final_delivery_request_rejects_idempotency_fingerprint_conflicts(self):
        key = uuid.uuid4()
        record = ensure_final_delivery_request(
            event=self.event,
            requested_by=self.organizer,
            operation=EmailDeliveryRequest.Operation.FINAL_CONFIRMATION,
            idempotency_key=key,
