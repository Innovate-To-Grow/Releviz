...
class ImpersonateLoginTests(APITestCase):
    ...
    def test_valid_token_returns_jwt_for_impersonated_member(self):
        ...
        response = self.client.post(
            "/authn/impersonate-login/", {"token": token.token}, format="json"
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("access", response.data)
        self.assertNotIn("refresh", response.data)
        self.assertIn("releviz_refresh", response.cookies)
        self.assertTrue(response.cookies["releviz_refresh"]["httponly"])
